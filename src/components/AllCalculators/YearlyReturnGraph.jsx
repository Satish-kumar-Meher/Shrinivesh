...
const YearlyReturnGraphSummary = () => {
  const { mode: darkMode } = useSelector((state) => state.screenMode);

  const data = Array.from({ length: 15 }, (_, i) => {
    const year = i + 1;
    const investment = year * 60000;
    const returns = investment * 0.7;
    return {
      year: `Year ${year}`,
      investment,
      returns,
            ...
            <div className="w-full h-96">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={data}>
                  ...
                  <YAxis stroke={darkMode ? "#ffffff" : "#333333"} />
                  <Tooltip
                    contentStyle={{ backgroundColor: darkMode ? "#0e1824" : "#ffffff", borderColor: darkMode ? "#10e2ea" : "#0e6371" }}
              ...
              <table className="min-w-full text-sm text-left border-collapse">
                <thead>
                  <tr className={`${labelColor} border-b ${darkMode ? "border-[#10e2ea]/30" : "border-[#0e6371]/20"}`}>
                    ...
                    <th className="px-4 py-2 font-semibold">Investment</th>
                ...
                <tbody>
                  {data.map((row, index) => (
                    <tr key={index} className="border-b last:border-none border-gray-500/10">
                      ...
                      <td className={`px-4 py-2 text-green-500`}>₹ {row.returns.toLocaleString("en-IN")}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
export default YearlyReturnGraphSummary;